...
static inline void	err_cmd_not_found(char *context, char *cmd)
{
	ft_eprintf("%s: %s: command not found\n", context, cmd);
}

static inline void	err_no_path_var(t_state *state, char *cmd)
{
	ft_eprintf("%s: %s: No such file or directory\n", context, cmd);
}

static inline void	err_1_errno(char *context, char *p1)
{
...
static inline t_ast_node	unexpected(char *context, t_parser *parser, t_ast_node ret, t_deque *tokens)
{
	t_token	t;
...
static inline void	critica_error_errno_context(char *context)
{
	ft_eprintf("[ERROR] %s\n", strerror(errno));
	exit(1);
}
