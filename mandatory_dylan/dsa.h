...
static inline void	deque_clear(t_deque *ret)
{
	ret->cap = 0;
	ret->start = 0;
	ret->len = 0;
...
static inline bool	vec_init(t_vec *ret, size_t elem_size)
{
	*ret = (t_vec){0};
	ret->elem_size = elem_size;
	return (true);
...
void	ast_postorder_traversal(t_ast_node *node, void (*fn)(t_ast_node *node))
{
	...
	{
		ast_postorder_traversal(vec_ctx_idx(&node->children, i), f);
		i++;
	}
	fn(node);
}
...
static inline t_ast_node(t_token t, int offset, int end_offset, t_tt tt)
{
	t_ast_node	ret;
