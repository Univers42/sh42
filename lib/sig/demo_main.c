...
/*   Updated: 2025/11/08 10:19:39 by dlesieur         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
typedef struct s_child { pid_t pid; int pipe_in[2]; int pipe_out[2]; char *leak_buf; int zombie; } t_child;
...
static void logf(const char *clr, const char *tag, const char *msg)
{
	if (g_verbose)
	{
...
static void set_protect_mode(int mode)
{
	...
	if (mode == 2)
	{
		...
		if (sigaction(SIGCHLD, &sa, NULL) < 0)
			logf(C_RED, "[ERR] ", "sigaction failed\n");
		else
			logf(C_GRN, "[CFG] ", "SA_NOCLDWAIT enabled (kernel reaps)\n");
	}
	else
	{
...
static int spawn_child(const char *label)
{
	...
	if (fcntl(c->pipe_out[0], F_SETFL, O_NONBLOCK) < 0)
		logf(C_RED, "[WARN] ", "fcntl O_NONBLOCK failed\n");
	
	g_child_count++;
	logf(C_BLU, "[SPAWN] ", "child created\n");
	return 0;
}
...
static void kill_child(int idx, int sig)
{
	...
	if (kill(c->pid, sig) < 0)
		logf(C_RED, "[KILL] ", "kill failed\n");
	else
	{
		if (sig == SIGKILL || sig == SIGTERM) 
			c->zombie = 1;
		logf(C_RED, "[KILL] ", "signal sent\n");
	}
}
...
int main(void)
{
	...
	{ 
		...
		{ 
			g_got_int = 0; 
			logf(C_YEL, "[SIGINT] ", "interrupt received\n");
			continue; 
		} 
		line[r] = '\0';
		...
		{ 
			g_leak_mode = 1; 
			logf(C_RED, "[CFG] ", "leak_mode=parent-leak (buffers will leak on crash)\n"); 
		}
		else if (starts_with(line, "leak off")) 
		{
			...
			g_leak_mode = 0; 
			logf(C_GRN, "[CFG] ", "leak_mode=clean (buffers freed properly)\n");
		}
